...
set(LITERT_VERSION "${LITERT_VERSION}" CACHE STRING "LiteRT version")
set(DALI_VERSION "${DALI_VERSION}" CACHE STRING "NVIDIA DALI version")
...
set(_dependency_root_candidates "")
if(DEFINED ENV{DEPENDENCY_ROOT} AND NOT "$ENV{DEPENDENCY_ROOT}" STREQUAL "")
    list(APPEND _dependency_root_candidates "$ENV{DEPENDENCY_ROOT}")
endif()
if(WIN32)
    if(DEFINED ENV{USERPROFILE} AND NOT "$ENV{USERPROFILE}" STREQUAL "")
        list(APPEND _dependency_root_candidates "$ENV{USERPROFILE}/dependencies")
    endif()
    if(DEFINED ENV{HOME} AND NOT "$ENV{HOME}" STREQUAL "")
        list(APPEND _dependency_root_candidates "$ENV{HOME}/dependencies")
    endif()
else()
    if(DEFINED ENV{HOME} AND NOT "$ENV{HOME}" STREQUAL "")
        list(APPEND _dependency_root_candidates "$ENV{HOME}/dependencies")
...
if(_dependency_root_candidates)
    list(GET _dependency_root_candidates 0 DEFAULT_DEPENDENCY_ROOT)
else()
    set(DEFAULT_DEPENDENCY_ROOT "${CMAKE_BINARY_DIR}/dependencies")
endif()
set(DEFAULT_DEPENDENCY_ROOT "${DEFAULT_DEPENDENCY_ROOT}" CACHE PATH "Default dependency installation root")
...
set(LITERT_DIR "${DEFAULT_DEPENDENCY_ROOT}/litert" CACHE PATH "LiteRT installation directory")
...
function(validate_backend_versions)
    ...
    foreach(MAPPING ${BACKEND_VERSION_MAPPING})
        ...
        elseif(VERSION_VAR_NAME STREQUAL "LITERT_VERSION")
            set(VERSION_VAR "${LITERT_VERSION}")
        elseif(VERSION_VAR_NAME STREQUAL "DALI_VERSION")
            set(VERSION_VAR "${DALI_VERSION}")
        else()
            set(VERSION_VAR "")
        endif()
...
message(STATUS "LiteRT: ${LITERT_VERSION}")
message(STATUS "DALI: ${DALI_VERSION}")
