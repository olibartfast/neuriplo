...
LlamaCppInfer::LlamaCppInfer(const std::string& model_path, bool use_gpu, size_t batch_size,
    ...
    : InferenceInterface{model_path, use_gpu, batch_size, input_sizes},
      model_(nullptr), ctx_llama_(nullptr), vocab_(nullptr), ctx_mtmd_(nullptr),
      model_loaded_(false) {
...
LlamaCppInfer::get_infer_results(const std::vector<std::vector<uint8_t>>& input_tensors) {
    if (!model_loaded_) throw std::runtime_error("llama.cpp model not loaded");
    if (input_tensors.empty()) throw std::runtime_error("No input tensors provided");
...
LlamaCppInfer::infer_text_only(const std::string& raw_prompt) {
    ...
    const int n_tokens = llama_tokenize(vocab_, prompt.c_str(), static_cast<int>(prompt.size()),
                                        tokens.data(), n_prompt_max, true, true);
    if (n_tokens < 0) throw InferenceExecutionException("Failed to tokenize prompt");
    ...
    return std::make_tuple(
        std::vector<std::vector<TensorElement>>{std::move(output)},
...
LlamaCppInfer::infer_multimodal(const std::string& raw_prompt, const std::vector<uint8_t>& image_bytes) {
    if (image_bytes.size() < 8) throw InferenceExecutionException("Image tensor too small");

    uint32_t nx, ny;
    std::memcpy(&nx, image_bytes.data() + 0, 4);
    std::memcpy(&ny, image_bytes.data() + 4, 4);
    const size_t expected = 8 + static_cast<size_t>(nx) * ny * 3;
    if (image_bytes.size() != expected) throw InferenceExecutionException("Image tensor size mismatch");
    ...
    if (!raw_bmp) throw InferenceExecutionException("Failed to create mtmd bitmap");
    ...
    if (tok_ret != 0) throw InferenceExecutionException("mtmd_tokenize failed: " + std::to_string(tok_ret));
    ...
    const int32_t eval_ret = mtmd_helper_eval_chunks(
        ctx_mtmd_, ctx_llama_, chunks.ptr.get(),
        /*n_past=*/0, /*seq_id=*/0, /*n_batch=*/512, /*logits_last=*/true, &n_past);
    if (eval_ret != 0) throw InferenceExecutionException("mtmd_helper_eval_chunks failed");
...
std::string LlamaCppInfer::apply_chat_template(const std::string& user_prompt) const {
    std::string tmpl_str;
    {
        ...
        if (len > 0) {
            ...
            if (ret >= 0) tmpl_str.resize(static_cast<size_t>(ret));
            else          tmpl_str.clear();
        }
    }
    if (tmpl_str.empty()) return user_prompt;
    ...
    if (tmpl_size <= 0) return user_prompt;
    ...
    if (result < 0) return user_prompt;
