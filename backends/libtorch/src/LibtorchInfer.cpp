...
LibtorchInfer::LibtorchInfer(const std::string& model_path, bool use_gpu, size_t batch_size,
    ...
    for (size_t i = 1; i < inputs.size(); ++i) {
        ...
        if (has_dynamic || (!input_sizes.empty() && (i - 1) < input_sizes.size())) {
            if (input_sizes.empty() || (i - 1) >= input_sizes.size()) {
                throw std::runtime_error("LibtorchInfer Initialitazion Error: Dynamic shapes found but no "
            ...
            if (has_dynamic) {
                ...
                if (provided_shape.size() != fixed_shapes.size() - 1) {
                    throw std::runtime_error("Provided shape size mismatch for input '" + name + "'. Expected " +
                                             std::to_string(fixed_shapes.size() - 1) + " dimensions, got " +
        ...
        if (tuple_type) {
            ...
            for (size_t j = 0; j < elements.size(); ++j) {
                ...
                if (!shapes || shapes->empty() ||
                    std::any_of(shapes->begin(), shapes->end(), [](int64_t s) { return s <= 0; })) {
...
LibtorchInfer::get_infer_results(const std::vector<std::vector<uint8_t>>& input_tensors) {
    ...
    if (input_tensors.size() != inputs_meta.size()) {
        throw std::runtime_error("Input tensor count mismatch. Expected " + std::to_string(inputs_meta.size()) +
    ...
    for (size_t i = 0; i < inputs_meta.size(); ++i) {
        ...
        torch::Tensor input = torch::from_blob(const_cast<uint8_t*>(input_data.data()), tensor_dims, options);
