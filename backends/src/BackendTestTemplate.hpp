 ...
 */
template <typename BackendClass> class BackendHybridTestBase : public AtomicBackendTest {
  protected:
  ...
  private:
    ...
    virtual std::vector<std::string> GetPossibleModelPaths() {
        return {"resnet18.onnx", "../resnet18.onnx", "model.onnx"};
    }

    virtual std::string GetDefaultModelPath() { return "resnet18.onnx"; }
  ...
  protected:
    ...
    PerformanceMetrics RunPerformanceBenchmark(size_t num_iterations = 100) {
        ...
        metrics.avg_inference_time_ms =
            std::accumulate(inference_times.begin(), inference_times.end(), 0.0) / num_iterations;
...
template <typename BackendClass> std::string BackendHybridTestBase<BackendClass>::model_path = "";

template <typename BackendClass> bool BackendHybridTestBase<BackendClass>::has_real_model = false;

template <typename BackendClass> bool BackendHybridTestBase<BackendClass>::model_discovery_done = false;

// Utility macros for consistent test naming
#define BACKEND_UNIT_TEST(TestClass, TestName) TEST_F(TestClass, Unit_##TestName)

#define BACKEND_INTEGRATION_TEST(TestClass, TestName) TEST_F(TestClass, Integration_##TestName)

#define BACKEND_PERFORMANCE_TEST(TestClass, TestName) TEST_F(TestClass, Performance_##TestName)

#define BACKEND_STRESS_TEST(TestClass, TestName) TEST_F(TestClass, Stress_##TestName)
...
#define STRESS_TEST_CONCURRENT_INFERENCE(TestClass)                                                                    \
    BACKEND_STRESS_TEST(TestClass, ConcurrentInference) { RunStressTest(4, 100); }
