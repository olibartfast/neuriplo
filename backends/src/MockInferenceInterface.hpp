...
class MockInferenceInterface : public InferenceInterface {
  public:
    ...
    void SetupDefaultExpectations() {
        ...
        ON_CALL(*this, get_last_inference_time_ms()).WillByDefault(testing::Return(5.0)); // Mock 5ms inference time

        ON_CALL(*this, get_total_inferences()).WillByDefault(testing::Return(total_inferences_));

        ON_CALL(*this, get_memory_usage_mb()).WillByDefault(testing::Return(memory_usage_mb_));

        ON_CALL(*this, clear_cache()).WillByDefault(testing::Invoke([this]() {
            memory_usage_mb_ = 10; // Reduced memory after cache clear
        }));
    }

    // Helper to create mock inference results
    std::tuple<std::vector<std::vector<TensorElement>>, std::vector<std::vector<int64_t>>> CreateMockInferenceResult() {
    ...
    void SetupErrorScenarios() {
        ...
        // Normal inference
        EXPECT_CALL(*this, get_infer_results(_)).WillOnce(Return(CreateMockInferenceResult()));

        // Error scenario
        EXPECT_CALL(*this, get_infer_results(_)).WillOnce(Throw(InferenceExecutionException("Mock inference error")));

        // Memory error
        EXPECT_CALL(*this, get_infer_results(_)).WillOnce(Throw(std::bad_alloc()));
    }
  ...
  protected:
    ...
    void ValidateInferenceResult(
        ...
        for (size_t i = 0; i < output_vectors.size(); ++i) {
            ...
            ASSERT_EQ(output_vectors[i].size(), expected_size) << "Output " << i << " size should match shape";
        }
    }
    ...
    void ValidateTensorElementTypes(const std::vector<TensorElement>& tensor) {
        for (const auto& element : tensor) {
            ...
            ASSERT_TRUE(std::holds_alternative<float>(element) || std::holds_alternative<int32_t>(element) ||
