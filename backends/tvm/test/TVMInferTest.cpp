...
protected:
    ...
    std::unique_ptr<MockTVMInfer> mock_infer;
    ...
    void SetUp() override {
            ...
            if (!model_path.empty()) {
                ...
                if (fs::exists(model_path)) {
                    ...
                    try {
                        ...
                        real_infer = std::make_unique<TVMInfer>(model_path, false, 1, input_sizes);
                        std::cout << "Using real TVM model: " << model_path << std::endl;
                    } catch (const std::exception& e) {
                        std::cout << "Failed to load real TVM model, falling back to mock: " << e.what() << std::endl;
                        has_real_model = false;
                    }
                }
            }
        ...
        if (!has_real_model) {
            mock_infer = std::make_unique<MockTVMInfer>();
...
TEST_F(TVMInferTest, IntegrationTest) {
    if (!has_real_model) {
        GTEST_SKIP() << "Skipping integration test - no real TVM model available";
    }
...
TEST_F(TVMInferTest, MockUnitTest) {
    if (has_real_model) {
...
TEST_F(TVMInferTest, GPUTest) {
    if (!has_real_model) {
        GTEST_SKIP() << "Skipping GPU test - no real TVM model available";
    }
