...
// Abstract Factory for the TVM runtime family: pairs the TVMInfer adapter with
// the default host allocator and tensor converter.
class TVMRuntimeFactory : public IBackendRuntimeFactory {
  public:
    std::unique_ptr<InferenceInterface> create_backend(const std::string& model_path, bool use_gpu, size_t batch_size,
        ...
        return std::make_unique<TVMInfer>(model_path, use_gpu, batch_size, input_sizes);
    }
    ...
    const char* name() const noexcept override { return "TVMRuntimeFactory"; }
};
