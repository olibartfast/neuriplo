...
TVMInfer::TVMInfer(const std::string& model_path, bool use_gpu, size_t batch_size,
    ...
    : InferenceInterface{model_path, use_gpu, batch_size, input_sizes}, module_handle_(nullptr), model_loaded_(false) {
    // Determine device
    if (use_gpu) {
...
TVMInfer::get_infer_results(const std::vector<std::vector<uint8_t>>& input_tensors) {
    ...
    if (input_tensors.size() != 1) {
        throw std::runtime_error("TVM backend currently supports only single input models, got " +
